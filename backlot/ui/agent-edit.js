...
async function load() {
  ...
  try {
    detail = await getJSON(
      `/api/project/${encodedProjectId}/agent-edit/${encodedRunId}`,
    );
    ...
    el("div", { class: "panel-body" },
      ...scenes.map((scene, index) => el("article", {
        ...
        el("div", { class: "scene-copy" },
          el("b", {}, scene.cut.title || scene.cut.text || scene.cut.terminalTitle || scene.id),
          el("small", {}, `${scene.cut.type} · ${fmtDuration(scene.duration_seconds)}`),
          el("code", {}, scene.id),
        ),
      )),
    ),
...
function renderProjectGraph() {
  ...
  return el("main", { class: "panel graph-panel" },
    el("section", { class: "hero" },
      ...
      el("h2", {}, working?.project.title || "Untitled project"),
      ...
      el("div", { class: "metrics" },
        ...
        el("div", {}, el("span", {}, "Timeline"), el("b", {}, fmtDuration(total))),
        el("div", {}, el("span", {}, "Operations"), el("b", {}, buildOperations().length)),
    ...
    el("section", { class: "timeline-section" },
      el("div", { class: "section-head" },
        ...
        el("span", {}, `${fmtDuration(total)} total`),
      ),
    ...
    el("section", { class: "operation-preview" },
      el("div", { class: "section-head" },
        ...
        el("span", {}, `${buildOperations().length} staged`),
      ),
      renderOperationList(),
    ),
  );
...
function renderInspector() {
  ...
  if (!scene) {
    return el("aside", { class: "panel inspector empty" }, "Select a scene.");
  }
...
async function submitDerivedRun(editor, summary) {
  const operations = buildOperations();
...
function renderSubmitBar() {
  ...
  const eligible = ["awaiting_human", "validated", "applied"].includes(status);
  const editorInput = el("input", {
    value: localStorage.getItem(EDITOR_KEY) || "",
    placeholder: "Editor identity",
  });
  editorInput.addEventListener("input", () => {
    localStorage.setItem(EDITOR_KEY, editorInput.value);
  });
  const summaryInput = el("input", {
    placeholder: "Describe the intended edit",
  });
  const button = el("button", {
    type: "button",
    class: "derive",
    disabled: !enabled || !eligible || !buildOperations().length,
  ...
  return el("footer", { class: "submit-bar" },
    enabled ? null : el("div", { class: "lock-message" },
    ...
    el("div", { class: "submit-meta" },
      el("b", {}, `${buildOperations().length} operations`),
